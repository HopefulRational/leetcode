...
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        TreeNode *cur = nullptr, *left = nullptr, *right = nullptr;
        if(root->val == p->val or root->val == q->val) cur = root;
        if(root->left) left = lowestCommonAncestor(root->left, p, q);
        if(root->right) right = lowestCommonAncestor(root->right, p, q);
        int cnt = (cur != nullptr) + (left != nullptr) + (right != nullptr);
        if(cnt >= 2) return root;
        return cur ? cur : (left ? left : right);
    }
};
```
