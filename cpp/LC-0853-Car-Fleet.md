...
public:
    int carFleet(int target, vector<int>& position, vector<int>& speed) {
        ...
        data.reserve(n);
        for(int i=0; i<n; ++i) {
            data.emplace_back(position[i], speed[i]);
        }
        
        sort(data.rbegin(), data.rend());

        int ans = position.size();
        double maxTimeTaken = (target - data[0].first) / (data[0].second * 1.0);

        for(int i=1; i<n; ++i) {
            double timeTaken = (target - data[i].first) / (data[i].second * 1.0);
            if(timeTaken <= maxTimeTaken) --ans;
            else maxTimeTaken = timeTaken;
        }
